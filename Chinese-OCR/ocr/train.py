...
class TrainDataGenerator(keras.callbacks.Callback):
    ...
    def build_word_list(self):
        ...
        for file in self.labelfiles:
            ...
            print(file)
            m = self.text_to_labels(gt)
            try:
                self.Y_len[i] = len(gt)
                self.Y_data[i, 0:len(gt)] = m
                self.X_text.append(gt)

            except:
                print(len(gt), len(m), gt, m)
...
def train(path='.', img_w=150, epochs=10, initial_epoch=0, max_string_len=10, batch_size=256,weights_path=None):
    # Input parameters
    img_h = 32
    ...
    viz_cb = VizCallback(output_dir="/home/chentong/chinese-ocr-master/ocr",
...
if __name__ == '__main__':
    train('/home/chentong/chinese-ocr-master/ocr/chn', img_w=300, epochs=500, initial_epoch=208, max_string_len=12,
          batch_size=256,weights_path='/home/chentong/chinese-ocr-master/ocr/ocr0.2.h5')
